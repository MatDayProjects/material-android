...
- Never request, generate, store, or use a keystore, certificate, private key, signing password, signer service, or guest image. Android artifacts remain unsigned in local builds and GitHub Actions.
- Every APK and AAB verification path must reject JAR signature metadata, APK Signing Blocks, detached `.idsig` files, generated signing material, and symbolic-link escapes. A generic `apksigner` failure is not proof of an unsigned artifact; validate archives and signing structures independently, validate AAB semantics with pinned `bundletool`, and publish APK/AAB files only from a post-verification staging directory.
