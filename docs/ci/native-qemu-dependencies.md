...
| `package-and-smoke-test` | `ubuntu-24.04` | Temurin JDK 17, Gradle wrapper, Android SDK platform 35, build-tools 35.0.0 with `zipalign` explicitly added to `PATH`, platform-tools, API 35 default x86_64 emulator | Native runtime artifacts from the two successful `build-runtime` matrix jobs | APK, versioned dependency assets, test XML/HTML, runtime manifests, emulator instrumentation result |
